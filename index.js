...
const button_adapta = document.getElementById("button_adapta");
button_adapta.addEventListener("click", (event) => {
    //根据当前的散图信息，自动化适配图集宽高
    let { length } = imageList;
    if (length === 0) {
        //没有图片，恢复初始大小
        canvasWidth = 300;
        canvasHeight = 300;
        ...
        //根据散图确定行列数量
        let splitNum = getSplitNum(length);
        if (!splitNum) {
            splitNum = getSplitNum(length + 1);
        }
        //得到划分的行列数
        let { row, col } = splitNum;
        //记录信息
        let widthMaxArr = [];
        let heightArr = [];
        //散图分配
        for (let i = 0; i < col; i++) {
            ...
            heightArr.push(maxHeight);
            //计算本行宽度
            let width = 0;
            for (let one of widthArr) {
                width += one;
            }
            //加上间隔
            width += (widthArr.length + 1) * space;
            widthMaxArr.push(width);
        }
        //计算高度
        canvasHeight = 0;
        for (let one of heightArr) {
            canvasHeight += one;
        }
        //加上间隔
        canvasHeight += (heightArr.length + 1) * space;
        //计算宽度
        canvasWidth = Math.max(...widthMaxArr);
    }
    input_CanvasWidth.value = canvasWidth;
    input_CanvasHeight.value = canvasHeight;
    updateCanvas();
...
/**获取一个数字对应的拆分的行列信息 */
const getSplitNum = function (total) {
    let row = 0;
    let col = 0;
    let sqr = Math.ceil(Math.sqrt(total));
    for (let i = sqr; i >= 2; i--) {
        if (total % i === 0) {
            col = i;
            row = total / i;
...
button_save.addEventListener("click", (event) => {
    ...
    imageList.forEach((img) => {
        str += `char id=${img.char.charCodeAt(0)} x=${img.x} y=${img.y} width=${img.width} height=${
            img.height
        } xoffset=${img.xoffset} yoffset=${img.yoffset} xadvance=${img.xadvance} \n`;
    });
    // console.log(str)
